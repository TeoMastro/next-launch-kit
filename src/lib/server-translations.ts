...
export async function getValidationTranslation(key: keyof MessageStructure['Validation']): Promise<string> {
  const cookieStore = await cookies();
  const locale = cookieStore.get('locale')?.value as 'en' | 'el' || 'en';
...
export async function getAuthTranslation(key: keyof MessageStructure['SignIn']): Promise<string> {
  const cookieStore = await cookies();
  const locale = cookieStore.get('locale')?.value as 'en' | 'el' || 'en';
